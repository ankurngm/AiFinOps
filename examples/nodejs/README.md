...
`using-openai-sdk.js`, `ollama-using-openai-sdk-with-attribution.js`, and
`openai-using-openai-sdk-with-attribution.js` need one extra package that this repo does **not**
install for you. Install it locally in this folder before running any of them:
...
`gpt-4o-mini`, this time billed directly to your OpenAI account. The `ollama-*` examples don't
cost anything, since they call a local model priced at `$0` by `config/modelPricing.json`'s
wildcard entry.
