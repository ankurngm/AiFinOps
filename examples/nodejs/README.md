...
`config/providerModelMap.json`. The `openai-*` examples additionally need `OPENAI_API_KEY` set in
`.env`, and the `anthropic-*` examples need `ANTHROPIC_API_KEY` set in `.env`.
...
| `openai-chat-completion-with-attribution.js`     | Same attribution call as above, but against OpenAI directly instead of via OpenRouter           | `OPENAI_API_KEY` in `.env`                                  | Yes — a real OpenAI call          |
| `openai-using-openai-sdk-with-attribution.js`    | The `openai` SDK example above, pointed at OpenAI directly instead of via OpenRouter            | `openai` package (see below), `OPENAI_API_KEY` in `.env`    | Yes — a real OpenAI call          |
| `anthropic-chat-completion-with-attribution.js`  | Same attribution call as above, but against Anthropic directly instead of via OpenRouter        | `ANTHROPIC_API_KEY` in `.env`                               | Yes — a real Anthropic call       |
| `anthropic-using-openai-sdk-with-attribution.js` | The `openai` SDK example above, pointed at Anthropic directly instead of via OpenRouter         | `openai` package (see below), `ANTHROPIC_API_KEY` in `.env` | Yes — a real Anthropic call       |
...
`using-openai-sdk.js`, `ollama-using-openai-sdk-with-attribution.js`,
`openai-using-openai-sdk-with-attribution.js`, and `anthropic-using-openai-sdk-with-attribution.js`
need one extra package that this repo does **not** install for you. Install it locally in this
...
cent — real spend on your OpenRouter account, not simulated. The `openai-*` examples also use
`gpt-4o-mini`, this time billed directly to your OpenAI account. The `anthropic-*` examples use
`claude-haiku-4-5-20251001`, Anthropic's smallest current model, billed to your Anthropic account.
The `ollama-*` examples don't cost anything, since they call a local model priced at `$0` by
`config/modelPricing.json`'s wildcard entry.
